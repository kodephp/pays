    ...
    WebhookCapableInterface
{
     ...
     *
     * 复用 {@see verifyNotify()} 的 MD5 验签逻辑（md5_key 后缀），但接收原始报文，
     * 不再依赖全局 `$_SERVER` / `php://input`。
     *
     * @param string $payload 原始请求体（form-urlencoded 或 JSON）
     * @param array<string, string> $headers 请求头（京东通知签名在报文体内，未使用）
     * @return bool
     */
    ...
    public function parseWebhook(string $payload): array
    {
        ...
        return [
            'gateway' => 'jd',
            'event_id' => $data['outTradeNo'] ?? null,
            'event_type' => $data['resultCode'] ?? $data['status'] ?? 'unknown',
            'data' => $data,
            'raw' => $payload,
        ];
    }
