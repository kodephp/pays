...
与对账单下载互补，用于账实核对与可用资金监控。微信支付 V3 / 支付宝 / Stripe / Revolut / Wise / Adyen / Xendit / PayPal / Payoneer 提供实时余额查询：
...
| Payoneer | ✅ `GET /v2/programs/{programId}/balance` | ❌ 报「无此方法」 | 项目账户实时余额（Basic(api_key:api_secret) 认证）；金额为主币种单位（如 "19669.36"），换算为分；响应兼容 `balance`/`available_balance`/`balance` 对象多种形态 |

> 能力开关：微信支付 V3 / 支付宝 / Stripe / Revolut / Wise / Adyen / Xendit / PayPal / Payoneer 在 `GatewayManifest` 中均声明 `CAP_BALANCE => true`，
